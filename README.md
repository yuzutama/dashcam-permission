...
Cursor / Codex / Claude のどれを使っていても運用しやすいように、リポジトリ内の補助スキルもできるだけ揃えています。
...
- X API の認証情報
- `yt-dlp`
- Windows の場合は、外部コマンドが `PATH` に通っていること
...
Windows でも動くように、主要な外部コマンド呼び出しは OS 差分を吸収する形にしています。
...
不足している API キーや外部コマンドがあると、ここで分かります。
...
| `npm run dev` | 管理画面を開く |
| `npm run doctor` | 必須設定と外部コマンドを確認する |
...
先に `npm run doctor` を実行すると確認しやすいです。
...
`yt-dlp`、`ffmpeg`、`ffprobe`、`codex`、`claude` を入れたあと、PowerShell を開き直してください。

それでもだめなら、`PATH` が通っていない可能性があります。まず `npm run doctor` を実行して不足項目を確認してください。
